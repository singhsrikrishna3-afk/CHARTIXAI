...
function OtherResultCard({ match, scanLabel, isBullish }) {
      ...
      return (
        <div className={styles.form}>
          ...
          <div className={styles.paramGrid}>
            ...
            <ParamInput label="Near Pivot %" value={params.near_pivot_pct || 5} onChange={v => setParams({...params, near_pivot_pct: v})} step={0.5} min={1} max={15} />
            <ParamInput label="Vol Dry-Up % (VDU)" value={params.vdu_pct || 75} onChange={v => setParams({...params, vdu_pct: v})} min={20} max={100} />
          </div>
          <label className={styles.checkboxLabel}>
...
export default function OtherScansPage() {
  ...
  async function handleScan() {
    ...
    try {
      const data = await api.runOtherScan({ scan_type: activeTab, timeframe, ...params });
      setResults(data);
      const cache = {};
  ...
  const isBullish = activeTab === "vcp" || (!params.direction || params.direction !== "down" && params.direction !== "bearish"
    ...
    <div className={styles.page}>
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>Breakout · Divergence · VCP Pattern · 52W H/L · Volume · Pivot · Gaps · Fibonacci · Range · Elliott Wave · Gann Swing</p>
        </div>
      </div>
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.settingsPanel}>
          ...
          <div className={styles.sectionTitle}>{tabInfo?.icon} {tabInfo?.label} Parameters</div>
                  ...
                  <div className="scan-results-grid" style={{ padding: "8px 16px" }}>
                    {results.matches.map((m, i) => (
                      <OtherResultCard key={i} match={{ ...m, chart_data: chartCache[m.symbol] || [] }}
                        scanLabel={tabInfo?.label} isBullish={isBullish} />
                    ))}
                  </div>
                ) : (
                  <div className={styles.resultsTable}>
                    <div className={styles.tableHead}>
                      <span>Symbol</span><span>Company</span><span>Close ₹</span><span>Change %</span><span>Volume</span><span>Chart</span>
                    </div>
                    {results.matches.map((m, i) => (
