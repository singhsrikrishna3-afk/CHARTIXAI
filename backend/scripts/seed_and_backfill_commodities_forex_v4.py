"""PEESTOCK — Seed and backfill commodities (COMEX USD & derived MCX INR) and forex segments from EOD history."""
...
COMMODITIES_INDEX = {
    ...
    "constituents": [
        ...
        {"symbol": "GOLD_MCX", "name": "Gold MCX Spot (INR)", "exchange": "MCX", "segment": "COMM"},
        {"symbol": "SI=F", "name": "Silver COMEX (USD)", "exchange": "COMEX", "segment": "COMM"},
        {"symbol": "SILVER_MCX", "name": "Silver MCX Spot (INR)", "exchange": "MCX", "segment": "COMM"},
...
def backfill_ohlcv(engine, iid, ticker, period="max"):
    """Download and ingest EOD data for a standard instrument."""
    try:
...
def backfill_derived_metals(engine, gold_id, silver_id):
    """Download GC=F, SI=F, and USDINR=X and mathematically derive domestic gold (10g) and silver (1kg) EOD prices in INR."""
    try:
        logger.info("Deriving domestic Gold (10g) and Silver (1kg) EOD prices in INR from global spot and USDINR rate...")
        
        gc = yf.download("GC=F", period="max", interval="1d", auto_adjust=True, progress=False)
        si = yf.download("SI=F", period="max", interval="1d", auto_adjust=True, progress=False)
        usdinr = yf.download("USDINR=X", period="max", interval="1d", auto_adjust=True, progress=False)
        
        # Clean MultiIndex columns
        for df in [gc, si, usdinr]:
            if isinstance(df.columns, pd.MultiIndex):
                df.columns = df.columns.droplevel(1)
        
        # Select Close and Volume series
        gc_df = gc[['Open', 'High', 'Low', 'Close', 'Volume']].dropna()
        si_df = si[['Open', 'High', 'Low', 'Close', 'Volume']].dropna()
        usdinr_df = usdinr[['Close']].dropna()
        
        # 1. Derive Gold MCX Spot (INR per 10g)
        # Gold COMEX is quoted in USD per Troy Ounce. 1 troy ounce = 31.1034768 grams.
        # Price per 10g in INR = (COMEX Price * USDINR Exchange Rate) / 31.1034768 * 10
        gold_merged = gc_df.join(usdinr_df, lsuffix='_gc', rsuffix='_fx', how='inner')
        
        gold_records = []
        for dt, row in gold_merged.iterrows():
            fx = float(row['Close_fx'])
            if not fx or pd.isna(fx):
                continue
            gold_records.append({
                "time": dt.date(),
                "iid": gold_id,
                "o": float(row['Open']) * fx / 31.1034768 * 10,
                "h": float(row['High']) * fx / 31.1034768 * 10,
                "l": float(row['Low']) * fx / 31.1034768 * 10,
                "c": float(row['Close_gc']) * fx / 31.1034768 * 10,
                "v": int(row['Volume']) if pd.notnull(row['Volume']) else 0
            })
        ...
        if gold_records:
            with Session(engine) as session:
                ...
                session.commit()
            logger.info(f"Successfully derived and ingested {len(gold_records)} EOD bars for GOLD_MCX (Gold MCX Spot INR per 10g)")

        # 2. Derive Silver MCX Spot (INR per 1kg)
        # Silver COMEX is quoted in USD per Troy Ounce. 1 troy ounce = 31.1034768 grams.
        # Price per 1kg in INR = (COMEX Price * USDINR Exchange Rate) / 31.1034768 * 1000
        silver_merged = si_df.join(usdinr_df, lsuffix='_si', rsuffix='_fx', how='inner')
        
        silver_records = []
        for dt, row in silver_merged.iterrows():
            fx = float(row['Close_fx'])
            if not fx or pd.isna(fx):
                continue
            silver_records.append({
                "time": dt.date(),
                "iid": silver_id,
                "o": float(row['Open']) * fx / 31.1034768 * 1000,
                "h": float(row['High']) * fx / 31.1034768 * 1000,
                "l": float(row['Low']) * fx / 31.1034768 * 1000,
                "c": float(row['Close_si']) * fx / 31.1034768 * 1000,
                "v": int(row['Volume']) if pd.notnull(row['Volume']) else 0
            })
        ...
        if silver_records:
            with Session(engine) as session:
                ...
                session.commit()
            logger.info(f"Successfully derived and ingested {len(silver_records)} EOD bars for SILVER_MCX (Silver MCX Spot INR per 1kg)")
            
        return len(gold_records) + len(silver_records)
    except Exception as e:
        logger.error(f"Error deriving metals: {e}")
    return 0
...
def main():
    ...
    # 0. Clean up incorrect and ETF-based precious metal tickers from the DB
    with Session(engine) as session:
        ...
        """))
        session.commit()
    logger.info("Cleaned up deprecated spot and ETF metal tickers from database.")
    ...
    for idx_meta in indices_to_process:
        ...
        for item in constituents:
            ...
            with Session(engine) as session:
                ...
                # Derived instruments (GOLD_MCX, SILVER_MCX) will be backfilled specifically
                if sym not in ("GOLD_MCX", "SILVER_MCX"):
    ...
    # 2. Backfill standard instruments
    if tickers_to_backfill:
        logger.info(f"Starting EOD backfill for standard instruments...")
    ...
    # 3. Backfill derived GOLD_MCX and SILVER_MCX prices
    gold_id = symbol_to_id.get("GOLD_MCX")
    silver_id = symbol_to_id.get("SILVER_MCX")
    if gold_id and silver_id:
        derived_rows = backfill_derived_metals(engine, gold_id, silver_id)
        total_backfilled_records += derived_rows

    # 4. Clean price spikes and non-positive prices
    ...
    # 5. Trigger EOD scans for the new instruments
    ...
    logger.info(f"🎉 Commodities and Forex Ingestion V4 Complete!")
