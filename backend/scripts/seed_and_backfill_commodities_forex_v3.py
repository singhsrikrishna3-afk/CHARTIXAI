"""PEESTOCK — Seed and backfill commodities (COMEX USD & MCX INR ETFs) and forex segments from IPO/inception."""
...
COMMODITIES_INDEX = {
    ...
    "constituents": [
        ...
        {"symbol": "GOLDBEES.NS", "name": "Gold MCX Spot (INR)", "exchange": "MCX", "segment": "COMM"},
        {"symbol": "SI=F", "name": "Silver COMEX (USD)", "exchange": "COMEX", "segment": "COMM"},
        {"symbol": "SILVERBEES.NS", "name": "Silver MCX Spot (INR)", "exchange": "MCX", "segment": "COMM"},
...
def main():
    ...
    # 0. Clean up incorrect tickers if they exist in the DB
    with Session(engine) as session:
        session.execute(text("""
            UPDATE instruments 
            SET is_active = 0 
            WHERE symbol IN ('XAUINR=X', 'XAGINR=X')
        """))
        session.execute(text("""
            DELETE FROM index_constituents 
            WHERE instrument_id IN (SELECT id FROM instruments WHERE symbol IN ('XAUINR=X', 'XAGINR=X'))
        """))
        session.commit()
    logger.info("Cleaned up unsupported spot metal tickers from database.")
    ...
    # 2. Historical backfill for new instruments (like GOLDBEES.NS and SILVERBEES.NS)
    ...
    logger.info(f"🎉 Commodities and Forex Ingestion V3 Complete!")
