"""PEESTOCK — Seed and backfill commodities and forex segments from IPO/inception."""
...
COMMODITIES_INDEX = {
    ...
    "name": "Commodities",
    "constituents": [
        {"symbol": "GC=F", "name": "Gold Futures (COMEX Proxy)", "exchange": "MCX", "segment": "COMM"},
        {"symbol": "SI=F", "name": "Silver Futures (COMEX Proxy)", "exchange": "MCX", "segment": "COMM"},
        {"symbol": "CL=F", "name": "Crude Oil Futures (NYMEX Proxy)", "exchange": "MCX", "segment": "COMM"},
        {"symbol": "NG=F", "name": "Natural Gas Futures (NYMEX Proxy)", "exchange": "MCX", "segment": "COMM"},
        {"symbol": "HG=F", "name": "Copper Futures (COMEX Proxy)", "exchange": "MCX", "segment": "COMM"},
        {"symbol": "ALI=F", "name": "Aluminium Futures (LME Proxy)", "exchange": "MCX", "segment": "COMM"},
        {"symbol": "ZNC=F", "name": "Zinc Futures (LME Proxy)", "exchange": "MCX", "segment": "COMM"},
        {"symbol": "LED=F", "name": "Lead Futures (LME Proxy)", "exchange": "MCX", "segment": "COMM"}
    ]
}
...
def main():
            ...
            with Session(engine) as session:
                session.execute(
                    text(
                        ...
                        "ON CONFLICT (symbol) DO UPDATE SET is_active = 1, segment = :seg, exchange = :exch"
                    ),
    ...
    # 2. Historical backfill for all registered instruments
    logger.info("Starting historical EOD backfill from inception...")
    all_tickers = []
    for idx_meta in indices_to_process:
        for item in idx_meta["constituents"]:
            all_tickers.append((symbol_to_id[item["symbol"]], item["symbol"]))
            
    for idx, (iid, sym) in enumerate(all_tickers):
        logger.info(f"[{idx+1}/{len(all_tickers)}] Backfilling EOD history for {sym}...")
    ...
    logger.info(f"🎉 Commodities and Forex Ingestion Complete!")
