...
def sync_yfinance(period="5y"):
    ...
    with Session(engine) as session:
        ...
        # Post-sync spike cleanup to resolve Yahoo Finance data freak errors
        logger.info("Cleaning non-positive prices (<= 0) from database...")
        ...
        """))
        logger.info("Cleaning high spikes from database...")
        session.execute(text("""
            UPDATE ohlcv_eod
        ...
        """))
        logger.info("Cleaning low spikes from database...")
        session.execute(text("""
            UPDATE ohlcv_eod
