...
async def list_trendlines(
    ...
    """List detected trendlines, optionally filtered by symbol, timeframe, and type."""
    ...
    if timeframe:
        q = q.where(Trendline.timeframe == timeframe.upper())
    if line_type:
        if line_type.lower() == "support":
            q = q.where(Trendline.line_type.in_(["support", "uptrend"]))
        elif line_type.lower() == "resistance":
            q = q.where(Trendline.line_type.in_(["resistance", "downtrend"]))
        else:
            q = q.where(Trendline.line_type == line_type)
